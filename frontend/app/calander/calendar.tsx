...
import {
  ...
  loadRecentColors,
  saveRecentColors,
} from "@/lib/utils/calendarStorageBackend";
...
export default function CalendarComponent() {
    ...
    sortedEvents.forEach((event) => {
      ...
      while (true) {
        ...
        const hasConflict = rowEvents.some((existing) => {
          const existingDays = getDaysBetween(
            existing.startDate,
            existing.endDate,
          );
  ...
  const handleDayClick = useCallback((dateKey: string, e: React.MouseEvent) => {
    ...
    const x = Math.max(
      margin,
      Math.min(e.clientX, window.innerWidth - popupWidth - margin),
    );
    const y = Math.max(
      margin,
      Math.min(e.clientY, window.innerHeight - popupHeight - margin),
    );
  ...
  const handleAddEvent = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (eventType === "multi") {
        ...
        startDate =
          multiStartDate <= multiEndDate ? multiStartDate : multiEndDate;
        endDate =
          multiStartDate <= multiEndDate ? multiEndDate : multiStartDate;
      } else {
      ...
      if (customColor && /^#[0-9A-Fa-f]{6}$/.test(customColor)) {
        const newRecentColors = [
          customColor,
          ...recentColors.filter((c) => c !== customColor),
        ].slice(0, 8);
          ...
          <>
            ...
            >
              <form onSubmit={handleAddEvent}>
                      ...
                      style={{
                        ...
                        color:
                          eventType === "single"
                            ...
                            : "var(--text-muted)",
                        boxShadow:
                          eventType === "single" ? "var(--shadow-sm)" : "none",
                      }}
                    >
                      Single Day
                    </button>
                      ...
                      style={{
                        ...
                        color:
                          eventType === "multi"
                            ...
                            : "var(--text-muted)",
                        boxShadow:
                          eventType === "multi" ? "var(--shadow-sm)" : "none",
                      }}
                    >
                      Multi-Day
                    </button>
                      ...
                      {EVENT_COLORS.map((color) => (
                        <button
                          ...
                          style={{
                            ...
                            boxShadow:
                              eventColor === color.value && !customColor
                                ? `0 0 0 2px var(--bg-primary), 0 0 0 4px ${color.value}`
                                : "none",
                          }}
                          title={color.name}
                        />
                ...
                {/* Actions */}
                <div
                  className="px-4 py-3 flex justify-between gap-2 border-t"
                  style={{ borderColor: "var(--border-divider)" }}
                >
                  {editingEventId ? (
                    <button
